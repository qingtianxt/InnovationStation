...
<div class="nav">
	...
	<ul class="nav-ul">
		...
		<li class="nav-li"><a href="#">信息审核</a></li>
	</ul>
</div>
	<div>
		<form action="supplyServlet?method=list&result=list"   method="post" id="form">
		<table  class="query-table" >
		  	...
		  	<tr>
		    	...
		    	<td>
		    		<select name="status">
		    			<option value="sh in (0,-1)">请选择</option>
		    			<option value="sh=0" <c:if test="${supplyPageBean.status.equals('sh=0') }">selected="selected"</c:if>>未审核</option>
		    			<option value="sh=-1"<c:if test="${supplyPageBean.status.equals('sh=-1') }">selected="selected"</c:if>>已退回</option>
		    		</select>
		    	</td>
			    <td height="21">所属高新技术类别：</td>
			    <td colspan="1">
		  	...
		  	<tr>
		    	...
		    	<td>
		    	<input name="startTime" type="text" id="time1" onclick="J.calendar.get({id:'time1'});" readonly="readonly" value="${supplyPageBean.startTime }"/>
		      	<td>截止时间：
		      	<td>
	...
	<div>
		<table class="result-table" style="margin:auto;border-collapse:collapse;width:100%">
			<tr>
			...
			<c:forEach varStatus="status" items="${SupplyList }" var="supply">
				<tr>
					...
					<td style="text-align:left;padding-left:15px;" title="${supply.mc }">${supply.mc }</td>
					<td style="text-align:left;padding-left:15px;">${supply.jslb }</td>
					<td>${supply.time }</td>
					...
					<td><a href="supplyServlet?method=getSupply&result=detail&id=${supply.id }">查看</a>|
						<a href="supplyServlet?method=getSupply&result=sh&id=${supply.id }">审核</a></td>
				</tr>
			</c:forEach>
			<tr>
...
<script type="text/javascript">
	function del(id){
		var url = "supplyServlet?method=del&result=list1&id="+id;		
		layer
		.open({
			content : '你确定要删除吗?',
			btn : [ '确认', '取消' ],
