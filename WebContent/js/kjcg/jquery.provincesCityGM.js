...
$.fn.ProvinceCityGM = function(){
	...
	//插入3个空的下拉框
	_self.append("<select id='selectGM1' name='selectgm1'></select>");
	_self.append("<select id='selectGM2' name='selectgm2'></select>");
	_self.append("<select id='selectGM3' name='selectgm3'></select>");
	//分别获取3个下拉框
	...
	$sel1.change(function(){
		...
		if(index1==0){	//当选择的为 “请选择” 时
			...
			$.each( GTGM[index1-1] , function(index,data){
				$sel2.append("<option value='"+data+"'>"+data+"</option>");
			});
			$.each( GCGM[index1-1][0] , function(index,data){
	...
	$sel2.change(function(){
		...
		$.each( GCGM[index1-1][index2] , function(index,data){
			$sel3.append("<option value='"+data+"'>"+data+"</option>");
		});
	});
	return _self;
